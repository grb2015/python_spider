...
'''
	breif :	读取上交所的A股名单，然后根据股票代码在akshare查其它数据后返回为一个df
'''

def Struct_df_SH_A(columns):
	df_SH = pd.DataFrame(columns=columns)
	df_SH_source = pd.read_excel('bourse_SH.xls')   # 读取上交所的excel
	df_SH["类别"] = "沪A股"
...
'''
	breif :	读取深交所的A股名单，然后根据股票代码在akshare查其它数据后返回为一个df
'''

def Struct_df_SZ_A(columns):
...
if __name__ == '__main__':
	rt = erea.get_division_info(310101)
	print(rt)
